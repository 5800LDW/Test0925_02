...
import kld.com.huizhan.activity.check.look.base.LookCheckBaseActivity;
import kld.com.huizhan.adapter.check.CheckedLookAdapter;
...
public class LookCheckedActivity extends LookCheckBaseActivity implements CheckedLookAdapter.OnContentClickListener {

    private String TAG = "LookPutInActivity";
    List<AssetsCheck> list = new ArrayList<>();
    CheckedLookAdapter adapter;
    ...
    public void getData() {
        super.getData();
//        List<AssetsCheck> l =  DataFunctionUtil.getAssetsCheckList();
//        if(l.size()!=0){
//            list = l;
//        }
        int totalNum = DataSupport.where("checkIsSucceed = ?", "已盘").count(AssetsCheck.class);
    ...
    private void getData2(int page) {
        ...
        if (page > 0) {
            List<AssetsCheck> l = DataSupport.where("checkIsSucceed = ?", "已盘") .limit(onePageShowItemNum).offset(excursionNum).find(AssetsCheck.class);
            for(int i=0;i<l.size();i++){
                l.get(i).setCheckIsSucceed("已盘");
            }
            if (l.size() != 0) {
                list.addAll(l);
            }
            ...
            List<AssetsCheck> l = DataSupport.where("checkIsSucceed = ?", "已盘") .limit(onePageShowItemNum).find(AssetsCheck.class);
            if (l.size() != 0) {
                for(int i=0;i<l.size();i++){
                    l.get(i).setCheckIsSucceed("已盘");
                }
...
//        }
    ...
    public void showConent() {
        ...
        adapter = new CheckedLookAdapter(list, this);
