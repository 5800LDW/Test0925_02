...
public class MaterialEditText extends EditText {
    ...
    private void init(AttributeSet attrs) {
        if(attrs!=null) {
            TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.MaterialEditText);
            // 获取设置的最大输入字符数，默认为-1，-1时不限制输入
            maxCount = array.getInteger(R.styleable.MaterialEditText_maxLength, -1);
            // 捕捉焦点后，下划线的初始颜色
            preLineColor = array.getColor(R.styleable.MaterialEditText_preLineColor, 0xee18b4ed);
            // 标签文字的颜色
            labelColor = array.getColor(R.styleable.MaterialEditText_labelColor, 0xff18b4ed);
            // 标签文字的内容
            labelText = array.getString(R.styleable.MaterialEditText_labelText);
            if(labelText==null)
                labelText = "";
            // 超过长度后的下划线颜色
            overLengthColor = array.getColor(R.styleable.MaterialEditText_overlengthColor, Color.RED);
